...
export function buildAcquisitionAnalysisInstructions(language: ResponseLanguage) {
  return [
    ...
    "Classify a claim only when its evidence status materially affects the decision, using one word: Verified, Benchmark, Estimated, Assumption, Unknown, or Recommendation, written in the report's own language. Use Benchmark ONLY when a real external reference actually supports that specific number -- a named comparable transaction, an industry report, or an [R#] registry entry (e.g. a valuation multiple, a target payback window, a retention benchmark, when you can point to where that figure comes from). If you produced the number yourself with no such external reference, it is never Benchmark -- label it Estimated (your own approximate figure) or Assumption (a planning input you chose to construct a scenario) instead, and never present it as a verified fact about this specific target company. Every numeric threshold, budget, or timeframe you name in ROI Analysis, IRR Analysis, Revenue/Cost Synergies, or the Final Investment Recommendation must carry this label directly next to the number, in the same clause or sentence -- never only as a disclaimer elsewhere in the field. Do not decorate every sentence with a label -- a label on every line stops carrying any signal.",
