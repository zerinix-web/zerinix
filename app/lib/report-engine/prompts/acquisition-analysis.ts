...
export const acquisitionAnalysisPrompts = {
  executiveAcquisitionSummary:
    "Write a real executive recommendation, not a generic hold/wait placeholder. Structure it as five short parts, each its own paragraph or tight block: (1) Transaction overview -- the target, transaction type, purchase price, and target ARR from the verified deal facts. (2) Main opportunity -- the single strongest reason this acquisition could work, grounded in the target's actual scale (customer count, employee count) and sector. (3) Main risks -- the two or three risks that most affect the decision, not a generic risk list. (4) Preliminary recommendation -- proceed, proceed conditionally, renegotiate, or walk away, stated plainly. (5) Conditions before closing -- the specific items (e.g. financial statements, customer contracts, security assessment) that must be reviewed before the recommendation becomes final. Identify the target, transaction type (asset purchase, stock purchase, merger), and parties without inventing absent identifiers -- a board-level summary of the deal, never a startup elevator pitch.",
  ...
  finalInvestmentRecommendation:
    "Write this as the report's single, final executive decision, not a research summary. State the call as one of exactly three words -- Proceed, Wait, or Reject -- and why, in language a board would use in a Monday decision meeting -- not a restatement of findings. Name the specific conditions attached to the call (e.g. what must be verified, reviewed, or resolved before or alongside proceeding). State the confidence level and the specific evidence gap it depends on. Name the one condition that would change the call. Never overstate certainty, never pad with generic caution language, and never restate findings already established earlier in the report -- only their decision implication belongs here.",
  sources:
...
export function buildAcquisitionAnalysisInstructions(language: ResponseLanguage) {
  return [
    ...
    "Never write 'More evidence required', 'Additional verification required', 'Need authoritative source', or any similarly bare, internal-sounding placeholder phrase anywhere in this report -- those read as system or pipeline language, not something a strategic advisor would say to a client. Instead, always name the specific thing that still needs review and frame it as a next action a client would actually take: 'Further review is recommended for [specific item]...' or 'Management should validate [specific item] before...' -- naming the exact item (e.g. the target's EBITDA, customer contracts, security posture), never a generic 'evidence' or 'source'.",
