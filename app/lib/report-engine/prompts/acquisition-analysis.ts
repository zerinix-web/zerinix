...
export const acquisitionAnalysisPrompts = {
  ...
  finalInvestmentRecommendation:
    "Write this as the report's single, final executive decision, not a research summary and never a bare 'Preliminary recommendation: Wait'. Never output only the word 'Pause', 'Reject', or 'Proceed' as this field's entire content -- a professional conditional recommendation always names its call AND its reasoning together. State the call as one of exactly three phrases -- Proceed with Conditions, Pause with Reasons, or Reject -- based on the current information available, and why, in language a board would use in a Monday decision meeting -- not a restatement of findings. When the call is Proceed with Conditions, name the specific conditions (e.g. what must be reviewed or resolved before or alongside proceeding); when it is Pause with Reasons, name the specific reasons driving the pause and what would resolve them; when it is Reject, name the specific, material finding that makes the deal unworkable as currently structured. Ground the reasoning in the deal's own facts and risks, not a bare confidence score. Name the one condition that would change the call. Never overstate certainty, never pad with generic caution language, and never restate findings already established earlier in the report -- only their decision implication belongs here.",
  sources:
