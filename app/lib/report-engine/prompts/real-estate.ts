...
export const realEstatePrompts = {
  ...
  comparableMarketEvidence:
    "Present only recent, geographically relevant comparable sale and rental listings or transactions found in external sources. Include date, location, asset similarity, area, asking or transaction price, rent when relevant, currency, unit price, occupancy evidence when available, URL, and limitations. If none are verified, write Unknown.",
  valuationRange:
    "Produce a valuation range only if verified location, asset or parcel size and unit, zoning/use, sufficient recent comparables, currency, and an explicit calculation method are all available. For income-producing commercial property, separately assess only supported purchase price, contractual or market rent, occupancy, operating costs, NOI, cap rate or yield, financing terms, and holding period; never infer a missing input. A supported valuation must include evidence-labeled lines for each applicable gate. Otherwise use the language-specific preliminary-valuation result, missing-gates heading, and evidence-acquisition-plan heading required by the system instructions, naming the document or authoritative source responsible for every missing gate. Never invent a purchase price, rent, occupancy, yield, financing term, or valuation.",
  legalRisks:
  ...
  scenarioAnalysis:
    "Provide three conditional decision paths in the report language. For Turkish use Olumsuz, Temel, and Olumlu senaryo; do not use English scenario labels. For income-producing property, vary rent, occupancy, yield, financing, or holding-period inputs only when those inputs are evidenced and explicitly show the decision impact. Do not assign financial values unless the valuation evidence gate is satisfied. Clearly label assumptions and triggers.",
  investmentScore:
