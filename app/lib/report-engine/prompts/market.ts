...
export const marketPrompts = {
  ...
  strategicRecommendations: {
    prompt:
      "Write this as the report's closing verdict, in the report's own language -- never the literal English words 'CEO Summary' as a heading or label -- derived only from market evidence. State plainly whether the evidence supports entering, piloting, or avoiding this market, and why -- 'avoid' or 'not yet' is a complete, acceptable answer when the evidence supports it; never default to an entry recommendation the evidence does not support. If the decision is anything short of a fully evidence-supported entry, keep every First 90 Days action bounded and reversible (validation, research, or a small gated pilot) -- never write hiring plans, national/full-scale launch language, or large irreversible spend for a market that has not cleared that bar. First 90 Days must contain exactly three concrete actions with owners and clear proof points before the next decision, and every one of those actions must be measurable, not generic advice: name the specific geography or segment, the budget or spend ceiling, the KPI that will be tracked, and the numeric or evidence-based success criterion that determines whether to continue -- never write an action like 'run a pilot' without those four specifics. Every budget figure, KPI target, timeline, or sample size in these actions must either be grounded in cited evidence (name the Evidence Tie) or be plainly labeled a '(Planning Assumption)' immediately after the figure -- never present an invented number as if it were a verified fact. Connect every conclusion to cited evidence and state when no supported opportunity exists. Do not include a founder score, founder roadmap, sales strategy, pricing strategy, or financial plan. Max 180 words.",
    maxTokens: 1200,
  },
  sources: {
    prompt:
