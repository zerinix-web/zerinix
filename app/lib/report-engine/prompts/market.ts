...
export const marketPrompts = {
  ...
  cagr: {
    prompt:
      "State only defensible CAGR evidence for the requested period or the closest sourced period. Include the calculation basis, period, geography, and source. Explain material differences between published forecasts. Never derive false precision from unsupported endpoints. If no defensible CAGR exists for the requested period/geography, check whether adjacent-market benchmark data is available first: a parent-category or adjacent-geography (Europe/OECD/regional) growth rate is a legitimate stand-in for the requested market's trajectory -- state it, mark it [Estimated], and explain in plain business language why it is a reasonable stand-in (same demand drivers, comparable maturity, or structurally linked category), without using internal terms like 'proxy', 'registry', or 'evidence graph'. Only when no such benchmark is available should you instead say so explicitly, name the missing input, state the confidence impact, and name what would need to be sourced. Max 150 words.",
    maxTokens: 1100,
  },
  marketSegmentation: {
    prompt:
