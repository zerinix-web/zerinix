...
function toDomainResearchEvidence(item: MarketEvidenceItem, id: string): DomainResearchEvidence {
  return {
    id,
    field: item.field,
    claim: item.claim,
...
export function buildMarketResearchV2Bundle({
  ...
  // CROSS-REPORT ISOLATION FIX -- evidence IDs used to be numbered by a
  // module-level counter that never reset, so IDs kept climbing across
  // every report generated by this process for its whole lifetime. That
  // never leaked one report's DATA into another (each bundle's own
  // evidence array only ever held that report's own items), but it was
  // still process-global mutable state with no reason to exist: a plain
  // local counter, recreated fresh on every call, produces the exact same
  // unique-within-this-bundle IDs without surviving past this function.
  let localEvidenceIdCounter = 0;
  const evidence = outcomes.flatMap((outcome) =>
    outcome.items.map((item) => {
      localEvidenceIdCounter += 1;
      return toDomainResearchEvidence(item, `market_v2_${item.field}_${localEvidenceIdCounter}`);
    })
  );
