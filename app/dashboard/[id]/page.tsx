...
// TASK #69A-18A -- ROOT CAUSE FIX: this file used to carry its own,
// byte-for-byte duplicate copy of BenchmarkIntelligencePanel (plus its
// two local helpers, getBenchmarkFitLocale/localizeBenchmarkFitValue)
// instead of importing the shared one -- a second, independently
// maintained implementation the #69A-18A ticket explicitly forbids
// ("no manual synchronization between sections"). Both copies carried
// the identical "Largest Gaps" bug (sourced from benchmarkScore.
// deviations, a numeric Benchmark Fit concept, instead of the
// canonical benchmarkFit.validationGaps evidence-gap list). Importing
// the one, now-fixed shared component guarantees this web surface and
// Planner.tsx's own BenchmarkIntelligencePanel usage can never drift
// apart again.
...
// copies lived here, in ReportPdfButton.tsx, and in Planner.tsx, which is
...
function ExecutiveSummaryVisual({
  ...
  // comment), so this is skipped entirely for that report kind, exactly
  // like every other investmentScore-derived value on this page.
  const executiveSnapshot = isMarketIntelligence ? null : buildExecutiveSnapshot(content, investmentScore);
  ...
  // CRITICAL FIX (Task #17B) -- confirmed live: extractFirstInsight below
  // reads the executiveSummary's OWN raw first sentence verbatim (e.g.
  // "Bottom Line — Decision: ENTER the U.S. ..."), completely bypassing
  // marketDecision above -- so this card could show a correctly-downgraded
  // MONITOR badge right next to its own supporting-text line still
  // literally reading "Decision: ENTER the U.S." one paragraph later.
  // Every free-text sentence sourced from this section (the "AI
  // Investment Score" supporting line and the "Executive Highlights"
  // fallback, both below) is laundered through the SAME canonical
  // resolver's own reconciliation helper before display, so this section
  // can never contradict its own badge, regardless of what the raw model
  // text says.
  ...
  const kpis = [
    ...
    },
    {
      ...
      // extractMetricValue(content, "Market") scanned the ENTIRE free-form
      // executive summary for any bare "Market"/"TAM" word followed by a
      // colon/dash, with no boundary awareness of sentence or quotation
      // structure -- it could start its capture mid-sentence, inside a
      // quoted clause, producing malformed fragments like
    ...
    },
    {
      ...
      //
      // TASK #69A-36 -- ROOT CAUSE FIX. Confirmed live: for Business Plan,
      // extractMetricValue(content, "Risk") had the identical unbounded-
      // scan defect as Market Signal above, producing truncated fragments
      // like `Execution readiness:...`. Replaced with
      // executiveSnapshot.riskLevel/.mainRisk -- the SAME canonical Risk
      // Level/Main Risk fields the "Executive Snapshot" section (a few
      // panels below, and PDF) already reads from this identical
      // buildExecutiveSnapshot call, so this tile can never disagree with
      // them about the dominant risk. Falls back to "Tracked" only when
      // executiveSnapshot itself could not be built at all (should not
      // happen for a real report; defensive only).
      label: "Risk Posture",
      value: isMarketIntelligence
          ...
          <div className="mt-5 flex items-end gap-4">
            ...
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-zinc-300">
                {reconcileExecutiveText(extractFirstInsight(content)) || "Executive signal is being assembled."}
              </p>
            </div>
          </div>
        </div>
        <div className="p-5">
          ...
          <div className="mt-4 rounded-3xl border border-white/10 bg-black/30 p-4 shadow-inner shadow-black/25">
            ...
            <div className="mt-3 grid gap-2">
              {(highlights.length > 0 ? highlights.map(reconcileExecutiveText) : [reconcileExecutiveText(extractFirstInsight(content))]).map((highlight) => (
...
function ReportSectionVisual({
  ...
  if (normalizedTitle.includes("strategic recommendation")) {
    ...
    // CRITICAL FIX (Task #17) -- confirmed live: this section's own raw
    // text can carry the model's own independently-written decision
    // verdict (see isRecommendationHeadingLine's own comment), generated
    // at the same time as, but never reconciled with, Executive
    // Summary's canonical decision. Rather than trusting that verdict
    // (brittle, since the model's exact wording varies report to
    // report), this reads the SAME canonical resolver every other
    // decision surface in this file already calls, against the SAME
    ...
    // for the single controlling evidence gap (when exactly one exists,
    // the SAME discipline Task #37/#38 already use) -- never a second,
    // independently derived threshold; only resolves when
    // resolveMarketIntelligenceDecisionThresholds/EvidenceGaps already
    // agree on a single controlling factor.
...
function getRiskIndicatorClass(level: string) {
  ...
  // CRITICAL FIX (Task #20) -- confirmed live: a risk dimension with NO
  // defensible severity signal (never evaluated, not "checked and found
  // safe") must never fall through to this function's own "Low" styling
  // -- the same teal, safe-looking treatment as a genuinely assessed Low
  // risk. That is exactly how an unresolved, decision-critical gap (e.g.
  // unvalidated SOM) ended up sitting next to a Risk Heatmap that looked
  // reassuring purely because its categories were never evaluated for
  // this report kind at all. Uses the same neutral, unclassified badge
  // treatment already established elsewhere in this file for "not yet
  // assessed" states, clearly distinct from Low/Medium/High.
...
function ExecutiveSnapshotPanel({
  ...
  // own topRisks[0] first (the same field the dashboard header tile
  // above already reads, see this file's own marketMainRisk at line
  // ~2067) so this panel's Main Risk can never disagree with either the
  // canonical state, the Executive Summary card, or the PDF cover's own
  // canonical-state-first marketMainRisk/marketTopRisks -- falling back
  // to this same prose extraction only when no canonical state exists.
  const marketMainRisk = isMarketIntelligence
    ? marketIntelligenceCanonicalState?.topRisks?.[0] ||
  ...
  const marketNextAction = isMarketIntelligence
    ? extractMetricValueFromAliases(section.content, localizedLabelVariants("immediateNextAction")) ||
      snapshot.nextAction
    : snapshot.nextAction;
  // CRITICAL FIX (Task #20) -- confirmed live against a REAL report:
  // snapshot.riskLevel (the fallback below) is buildExecutiveSnapshot's
  // own generic inferRiskLevel(content, ["risk", "validation", "cac",
  // "funding", "execution", ...]) full-content scan -- for this exact
  // report it happened to return "High" only because the unrelated word
  // "unresolved" appears somewhere else in the executive summary, not
  // because of any principled Market-Intelligence-specific derivation.
  // A coincidental keyword match is not a defensible severity signal --
  // when this section's own resolved Main Risk sentence states no
  // explicit severity, the honest state is "Validation Needed", never a
  // guessed Low/Medium/High.
  const marketRiskLevel = isMarketIntelligence
    ...
    : snapshot.riskLevel;
  // CRITICAL FIX (Task #20) -- confirmed live: snapshot.riskHeatmap (the
  // fallback below) is built entirely from Business-Plan/startup risk
  // categories ("CAC", "Capital efficiency", "Execution", ...) that
  // Market Intelligence's own prompts never discuss -- every category
  // silently defaulted to "Low" purely because its keyword never
  // appeared, not because it was ever actually assessed. That produced
  // the exact reported contradiction: a Risk Level tile correctly
  // showing "High" (SOM unresolved) sitting directly above a heatmap of
  // five unrelated "Low" ratings that look reassuring but were never
  // evaluated at all. Rebuilt from the SAME canonical Top 3 Risks list
  // this section's own Main Risk tile already reads (real, per-item
  // severity via extractRiskLevel, never a keyword-presence guess); when
  // that structured list isn't available (a legacy/cached executive
  // summary predating the canonical banner, this exact report's case),
  // falls back to a single row reusing the ALREADY-RESOLVED Main
  // Risk/Risk Level above -- the heatmap can never disagree with the
  // tile next to it because both now read the identical source.
